...
app.post("/api/get_by_age_type", async (req, res) => {
    let { province_code, amphoe_code, tambon_code, age, type } = req.body
    let connection = await oracledb.getConnection(dbConfig);
    let where;
    if (!tambon_code) {
        where = `WHERE CONCAT(mda.PROVINCE_CODE,mda.DISTRICT_CODE) = '${amphoe_code}'`;
    ...
    let sql = `SELECT bp.PROVINCE_CODE, bp.PROVINCE_NAME, 
        ...
        WHERE bp.PROVINCE_CODE = '40' AND ((SELECT EXTRACT(YEAR FROM SYSDATE) FROM DUAL)-(SUBSTR(mdp.BIRTH_DATE,-4)-543)) < 50
